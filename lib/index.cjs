...
function getDifficultyMods(ruleset, mods) {
...
async function downloadFile(path, options) {
  const downloader = new osuDownloader.Downloader(path);
...
async function parseCustomBeatmap(url, hash) {
...
function parseBeatmapData(data, hash) {
  ...
  if (hash && hash !== md5__default['default'](stringified)) {
...
async function parseScoreData(data, hash) {
  if (hash && hash !== md5__default['default'](data)) {
    throw new Error('Wrong beatmap file!');
  }
...
class BeatmapCalculator {
  ...
  _simulateScores(beatmap, values) {
    if (!values) {
      return [];
    }

    const isForMania = beatmap.mode === exports.GameMode.Mania;
  ...
  _getRuleset({ ruleset, rulesetId }) {
    return ruleset ?? getRulesetById(rulesetId);
  }
  async _getBeatmap(ruleset, options) {
    if (options.beatmap) {
      return options.beatmap;
    }
