import { BeatmapInfo, HitType, ScoreInfo, Beatmap, ModType, ScoreRank, StrainSkill } from 'osu-classes';
import { StandardDifficultyCalculator, StandardRuleset, StandardDifficultyAttributes } from 'osu-standard-stable';
import { TaikoDifficultyCalculator, TaikoRuleset, TaikoDifficultyAttributes } from 'osu-taiko-stable';
...
class ScoreCalculator {
  ...
  async calculate(options) {
    if (this._checkPrecalculated(options)) {
      return await this._processPrecalculated(options);
    }
  ...
  async _processPrecalculated(options) {
  ...
  async _getScoreInfo(options, attributes) {
    const { scoreInfo, replayURL } = options;

    if (scoreInfo) {
      return toScoreInfo(scoreInfo);
    }
...
export { BeatmapCalculator, GameMode, ScoreCalculator, ScoreSimulator, calculateAccuracy, calculateDifficulty, calculatePerformance, calculateRank, createBeatmapAttributes, createBeatmapInfo, createDifficultyCalculator, downloadFile, generateHitStatistics, getRulesetById, getRulesetIdByName, getValidHitStatistics, parseBeatmap, parseScore, scaleTotalScore, toCombination, toDifficultyAttributes, toDifficultyMods, toScoreInfo };
