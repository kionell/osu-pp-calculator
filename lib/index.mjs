import { BeatmapInfo, HitType, Beatmap, ScoreInfo, MathUtils, ModType, ScoreRank, StrainSkill, Score } from 'osu-classes';
...
class ScoreCalculator {
  ...
  async calculate(options) {
    ...
    if (!attributes || !beatmapMD5 || !ruleset || !score || !difficulty || (isPartialDifficulty && !options.fix)) {
      ...
      score ?? (score = await this._createScore(options, attributes));
...
export { BeatmapCalculator, GameMode, ScoreCalculator, ScoreSimulator, calculateAccuracy, calculateDifficulty, calculatePerformance, calculateRank, calculateTotalHits, createBeatmapAttributes, createBeatmapInfo, createDifficultyCalculator, downloadFile, generateHitStatistics, getRulesetById, getRulesetIdByName, getValidHitStatistics, parseBeatmap, parseScore, scaleTotalScore, toCombination, toDifficultyAttributes, toDifficultyMods, toScoreInfo };
